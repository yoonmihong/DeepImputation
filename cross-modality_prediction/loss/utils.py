...
def discriminator_loss(D, y, fake_y, use_lsgan=True):
    ...
    if use_lsgan:
        ...
        error_fake = tf.reduce_mean(tf.math.square(D(fake_y)))
    else:
        # use cross entropy
...
def conditioned_discriminator_loss(D, condition, y, fake_y, use_lsgan=True):
    ...
    if use_lsgan:
        ...
        # use cross entropy
        error_real = -tf.reduce_mean(safe_log(D(condition, y)))
        error_fake = -tf.reduce_mean(safe_log(1 - D(condition, fake_y)))
    loss = (error_real + error_fake) / 2
    return loss
...
def conditioned_generator_loss(D, condition, fake_y, use_lsgan=True):
    ...
    if use_lsgan:
        ...
        # heuristic, non-saturating loss
        loss = -tf.reduce_mean(safe_log(D(condition, fake_y))) / 2
    return loss
...
def discriminator_loss_slim(D_y, D_fake_y, use_lsgan=True):
    ...
    if use_lsgan:
        ...
        # use cross entropy
        error_real = -tf.reduce_mean(safe_log(D_y))
        error_fake = -tf.reduce_mean(safe_log(1 - D_fake_y))
    loss = (error_real + error_fake) / 2
    return loss
...
def generator_loss_slim(D_fake_y, use_lsgan=True):
    ...
    if use_lsgan:
        ...
        # heuristic, non-saturating loss
        loss = -tf.reduce_mean(safe_log(D_fake_y)) / 2
    return loss
